...
problem_scrambled$probYN <- with(problem_scrambled, ifelse(`probYN` == 'Ok'| `probYN` == 'FÃ¶redÃ¶mligt handlagd'|
 `probYN` == 'ok' & is.na(problem_scrambled$probYN) == FALSE, 'Yes', 'No'))
...
# Gör om till Na
calb<-calc %>% replace_with_na(replace = list('rr_rts'=5))
cald<-calb %>% replace_with_na(replace = list('sbp_rts'=5))
cale<-cald %>% replace_with_na(replace = list('gcs_sum'=5))
calh<-cale %>% replace_with_na(replace = list('pt_age_yrs'=11))
...
library("questionr")
sew<-as.data.frame(freq.na(sega))
...
ae$ISS<-as.numeric(ae$ISS)
ae$dt_ed_first_ct<-as.numeric(ae$dt_ed_first_ct)
ae$dt_ed_emerg_proc<-as.numeric(ae$dt_ed_emerg_proc)
ae$pt_age_yrs<-as.numeric(ae$pt_age_yrs)
...
plot(pt_age_yrs ~ probYN, data=ae, col="steelblue")
...
library(knitr)
kable(sewe[,1:2], format = "markdown")
```
