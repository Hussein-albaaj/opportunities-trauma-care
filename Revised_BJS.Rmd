...
combined.data <- merge(fmp, combined.data, by = "id", all.x = FALSE)

combined.datasets<-filter(combined.data, as.Date(strptime(combined.data$Ankomst_te.x, format = "%Y%m%d %H:%M")) >="2017-01-01")

# Split name column into Date and Time
combined.datasets[c('Datum', 'Time')] <- str_split_fixed(combined.datasets$Ankomst_te.x, ' ', 2)
combined.datasets$Jour <- with(combined.datasets, ifelse(combined.datasets$Time >= "08:00" & combined.datasets$Time <= "16:59", "Yes", "No"))
...
poij<-subset(swee, DOA != "Yes")

poij$dt_ed_first_ct <- as.numeric(poij$dt_ed_first_ct)

# Gör om missing i Time to CT till 1
poij$dt_ed_first_ct[poij$dt_ed_first_ct>=0 & poij$dt_ed_first_ct<=30]<-1
poij$dt_ed_first_ct[poij$dt_ed_first_ct>30 & poij$dt_ed_first_ct<=60]<-2
poij$dt_ed_first_ct[poij$dt_ed_first_ct>60 & poij$dt_ed_first_ct<=90]<-3
poij$dt_ed_first_ct[poij$dt_ed_first_ct>90]<-4
poij$dt_ed_first_ct[is.na(poij$dt_ed_first_ct)]<-"not_done"

# Egen datafram för räkna missing data
fake.var<-c("Gender","res_survival","intub","host_care_level","rr_rts","sbp_rts","gcs_sum", "Jour", "Weekend", "dt_ed_first_ct")
rak.var<-c("id","ofi","Gender","res_survival","intub","host_care_level","rr_rts","sbp_rts","gcs_sum","Jour", "Weekend", "dt_ed_first_ct")
cont.var<-c("ISS","pt_age_yrs")
cont.procedure.var<-c("ISS","pt_age_yrs","dt_ed_emerg_proc")
cat.var<-c("ofi","Gender","res_survival","intub","host_care_level","rr_rts","sbp_rts","gcs_sum","Jour", "Weekend", "dt_ed_first_ct")

# Gör ny dataframe för att räkna missing data
ba<-select(poij,rak.var,cont.var)
kl<-select(poij,fake.var,cont.var)
na.kl<-na.omit(kl)
...
aa$dt_ed_first_ct <-
  factor(aa$dt_ed_first_ct, levels=c("1","2","3","4","not_done"),
         labels=c("0-30","31-60","61-90",">90","No CT"))

label(aa$ofi)      <- "Outcome"
label(aa$Jour)      <- "Time of the day"
...
opo<- as.data.frame(table1(~ Gender + pt_age_yrs + res_survival + host_care_level + intub + ISS + rr_rts + sbp_rts + Jour + Weekend + gcs_sum + dt_ed_first_ct | ofi, data=aa))
...
app.ba<-select(poij,rak.var,cont.procedure.var)
app.kl<-select(poij,fake.var,cont.procedure.var)
app.calc<-app.ba
...
app.aa$dt_ed_first_ct<-as.factor(app.aa$dt_ed_first_ct)
...
app.aa$gcs_sum <-
  factor(app.aa$gcs_sum, levels=c("3","2","1","5"),
         labels=c("Mild: 13-15","Moderate: 9-12","Severe: 3-8","Missing"))
...
table1_t <- table1(~ pt_age_yrs + Gender + res_survival + host_care_level + intub + ISS + rr_rts + sbp_rts + gcs_sum + Jour + Weekend + dt_ed_first_ct | ofi, data=aa, longtable = TRUE, caption = "Sample characteristics")
...
tbl_merge_test <-
    tbl_merge(tbls = list(aa %>%
                              select(ofi, pt_age_yrs, Gender, res_survival, rr_rts, sbp_rts, gcs_sum, intub, Jour, dt_ed_first_ct, Weekend, host_care_level, ISS) %>%
                              tbl_uvregression(
                                  method = glm,
                                  y = ofi,
...
k.jour = ddply(k.jour, .(Jour), transform, percent = n/sum(n) * 100)
k.jour = ddply(k.jour, .(Jour), transform, pos = (cumsum(n) - 0.5 * n))
k.jour$label = paste0(sprintf("%.0f", k.jour$percent))
...
e.jour<-k.jour[!(k.jour$ofi=="No opportunity for improvement"),]
op.jour <- ggplot(e.jour, aes(x = Jour, y = percent, fill = ofi)) +
    geom_bar(position = position_stack(), stat = "identity", width = .7) +
    geom_text(aes(label = label), position = position_dodge(width = 1),
...
op.ct <- ggplot(e.ct, aes(x = dt_ed_first_ct, y = percent, fill = ofi)) +
    geom_bar(position = position_stack(), stat = "identity", width = .7) +
    geom_text(aes(label = label), position = position_dodge(width = 1),
    vjust = -0.5, size = 3.5) + labs(y = "%", x = "Time to first CT") + guides(fill=guide_legend(title="OFI")) + theme(axis.text.x = element_text(angle = 45, vjust = 0.2, hjust=0.3, size=8, colour = "black")) + ylim(0, 15)
op.ct

new.bar <- ggarrange(op.sex, op.res ,op.jour, op.weekend, op.ct, op.intub,
          labels = c("A", "B", "C", "D","E","F"),
