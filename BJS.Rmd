...
# Introduction
Physical trauma is one of the leading causes of mortality and morbidity in all age groups [@David2021;@Ono2021]. There are approximately 4.5 million global deaths each year due to trauma [@James2020], and it is determined to be one of the highest ranked in disease burden worldwide, measured by disability-adjusted life years [@Murray1996;@Haagsma2015]. Trauma is also one of the most prevalent reasons for admission to the Intensive care unit (ICU) [@Prin2016], and is therefore considerably resource-intensive.

Patient outcomes are primarily dependent on the quality of care provided [@Dogrul2020], as observed in the decline of deaths by organ dysfunction after the adaptation of various treatment strategies and improvement in critical care [@Ghorbani2014;@Kahl2013]. Advances in trauma care are the leading cause of the decreased number of preventable deaths according to the American College of Surgeons Committee [@Committee2013]. In a teaching hospital in Tehran, reviews of all trauma cases in one year displayed inappropriate care in 45% of all deaths and implied that approximately 26% of all trauma deaths were preventable [@Zafarghandi2003]. Other studies have estimated preventable and potentially preventable trauma death rates up to 60% [@Konadu2020]. The corresponding local rates in Stockholm are estimated to 4% in total [@Ghorbani2018]. 
...
### Study design
This retrospective cohort study used data from the Karolinska University Hospital trauma registry (TrR), a part of the Swedish National Trauma Registry (SweTrau) [@Swetrau2020], and the local trauma care quality database (TrcQD). Established in 2004, Swetrau includes information of pre-hospital, hospital, and post-hospital care in accordance with the Utstein template [@Utstein2009]. Factors such as demographics, vital signs, time to procedure and time to intervention are registered [@Dick1999]. The TrcQD include the presence of OFI and consists of all trauma patients treated at Karolinska University Hospital from the year of 2017. The registries where linked and possible factors associated with opportunities for improvement were extracted. Bi- and multivariable logistic regression were used to determine association with the presence of OFI.

### Setting
In Sweden, pre-hospital care is managed by specialized nurses, emergency medical technicians and specially trained physicians. The trauma patient is triaged by the emergency medical services personnel at the scene, according to acuity. Patients are prioritized according to vital signs and mechanism of injury. If one or more of the trauma one criteria are met then the patient is treated as a priority one [@traumalarmskriterier2017]. All trauma patients in Stockholm with priority one are transported to Karolinska University Hospital to receive care by dedicated trauma teams. These teams consist of a trauma surgeon, an anaesthetist, an orthopaedic surgeon, a radiologist and specialized nurses. Karolinska University Hospital in Solna is a level one trauma center, with direct access to radiology, intervention, operation, intensive care and consultants in all associated specialties [@Social2015;@NKS2020].

Fig.\@ref(fig:flowchart-process) describes the process of trauma cases from arrival until the inclusion in morbidity and mortality conferences. Cases are selected for morbidity and mortality conferences by specialized nurses mainly based on audit filters. These filters in Karolinska University Hospital are systolic blood pressure under 90, GCS less than 9 and not intubated, ISS more than 15 but not admitted to the ICU, time to acute intervention more than 60 minutes, time to computed tomography more than 30 minutes, and death within 30 days after trauma. In some cases, second opinion is needed by a colleague. If there is a possibility of OFI, the case is sent to the morbidity and mortality review. Patients that die within 30 days are automatically sent to these reviews without the inspection made by nurses. Cases are then reviewed by experienced specialists from all the disciplines and professions involved in the trauma team. OFI in the care of the patient are identified when present and registered to the TrcQD. The presence of OFI is a consensus decision among all participants of the conference The absence of OFI is also registered. A plan to solve the problems identified is also presented by the board.
...
### Participants
The TrR includes all patients admitted with trauma team activation, regardless of ISS, as well as patients admitted without trauma team activation but found to have an ISS of more than 9. The previously mentioned audit filters are used to select patients for morbidity and mortality reviews. The inclusion criteria for this study are patients included in the TrR, TrcQD and aged at least 15 years. Patients with missing data in any of the covariates or outcome with the exception of GCS, respiratory rate, and systolic blood pressure were excluded. Patients under the age of 15 were also excluded since the trauma care differs for those patients at Karolinska University Hospital. Data was collected for `r nrow(combined.datasets)` patients registered between 2017 and 2021.

### Variables

**Study outcome**
The outcome variable is the presence of opportunities for improvement, as labeled by the morbidity and mortality review board, and defined as a binary variable with the levels "Yes - At least one OFI identified" and "No - No opportunities for improvement identified". Presence of opportunities for improvement is defined as preventable error made in a specific patient case. Data on this outcome waas extracted from the TrcQD. 

**Predictors**
The research team selected factors to be further examined through discussion, partly based on locally used audit filters, standard epidemiological factors and factors frequently registered in Swetrau. Selected factors to be further examined are divided into categorical and continuous variables. The categorical variables are sex, survival after 30 days, highest hospital care level, GCS, respiratory rate, systolic blood pressure and if the patient was intubated. The continuous variables are age, ISS, time from arrival at the hospital until first CT and time from arrival at the hospital until first intervention.
...
All patients in the TrcQD were analysed. Descriptive data is used to present the baseline characteristics. Bivariable logistic regression was used to determine unadjusted association, and multivariable logistic regression was used to determine adjusted association with the presence of OFI. Results are presented using a confidence interval of 95% and a significance level of 5%. The programming language R is used for all analyses [@Rstudio2022]. All statistical analysis were first done on synthetic data and later implemented on the data collected from the TrR and the TrcQD to ensure objectivity.

# Results
The baseline characteristics of included patients are presented in Table \@ref(tab:sample-characteristics). Out of `r nrow(combined.datasets)` patients included in TrR between 2017 and 2021, `r nrow(combined.datasets)-nrow(na.kl)` patients were excluded due to missing data and age under 15, leaving a total of `r nrow(na.kl)` patients eligible for the study. The most prevalent reason for exclusion was missing data in the factor `r rownames(sewe[sewe$missing == sewe[1,1],])` with `r sewe[1,1]` patients, `r round(sewe[1,1]/nrow(combined.datasets)*100, digits = 2)`% of patients registered. The number of patients excluded due to missing data in each factor is recorded in Table \@ref(tab:missing-data). In the TrcQD `r nrow(combined.datasets)-sum(is.na(combined.datasets$ofi))` of the patients were applicable for the study. After the combination of data from the TrR and the TrcQD and the removal of unqualified patients, a study group of `r nrow(aa)` is produced. Among the `r nrow(aa)` included patients, `r opo[3,4]` were male. The median age [Min, Max] for all patients was `r opo[7,4]`. The survival rate after 30 days among included patients was `r round(sum(aa$res_survival=='Alive')/nrow(aa)*100, digits = 2)`%.
