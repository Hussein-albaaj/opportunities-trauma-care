...
fake.var<-c("Kön","res_survival","intub","host_care_level","rr_rts","sbp_rts","gcs_sum")
rak.var<-c("id","ofi","Kön","res_survival","intub","host_care_level","rr_rts","sbp_rts","gcs_sum")
cont.var<-c("ISS","dt_ed_first_ct","pt_age_yrs")
cont.procedure.var<-c("ISS","dt_ed_first_ct","pt_age_yrs","dt_ed_emerg_proc")
cat.var<-c("ofi","Kön","res_survival","intub","host_care_level","rr_rts","sbp_rts","gcs_sum")
...
sex<-glm(ofi~Kön,data = aa,family = 'binomial')
summary(sex)
...
opo<- as.data.frame(table1(~ Kön + pt_age_yrs + res_survival + host_care_level + intub + ISS + rr_rts + sbp_rts +
...
app.aa$Kön<-as.factor(app.aa$Kön)
...
app.aa$Kön <-
  factor(app.aa$Kön, levels=c("M","K"),
         labels=c("Male",
                  "Female"))
...
rownames(sewe)[rownames(sewe) == "Kön"] = "Sex"
```
