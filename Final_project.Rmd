...
missing_table <- knitr::kable(sewe[,1:2], format = "markdown", caption = "Missing values per variable")
...
Systolic blood pressure and respiratory rate are registered to the database as either a continuous or a categorical value according to the Revised Trauma Score (RTS), but never both. The decision to convert the continuous values, if registered, into corresponding categorical value and treat them all as categorical was therefore made. All vital signs had to be treated similar and GCS will therefore be divided into categories based on RTS [Table REF-table.2]. 
...
The baseline characteristics of included patients are presented in Table 1. Out of `r nrow(xy)` included in the trauma registry `r nrow(xy)-nrow(na.kl)` were excluded due to missing data and age under 15, leaving a total of `r nrow(na.kl)` cases applicable for the study. The most prevalent reason for exclusion was missing data in the variable `r rownames(sewe[sewe$missing == sewe[1,1],])` with `r sewe[1,1]` patients, `r round(sewe[1,1]/nrow(xy)*100, digits = 2)`% of patients registered. In the trauma care quality database `r nrow(problem_scrambled)` patients were registered, although `r sum(is.na(problem_scrambled$probYN))` had to be excluded due to unidentified outcome, leaving `r nrow(problem_scrambled)-sum(is.na(problem_scrambled$probYN))` cases applicable for the study. After the combination of data from the trauma registry and the trauma care quality database and the removal of unqualified cases, a study group of `r nrow(aa)` is produced. Among the `r nrow(aa)` included patients, `r opo[3,4]` were male. The mean age (standard deviation) for all patients was `r opo[6,4]`. The survival rate after 30 days among included patients was `r round(sum(aa$res_survival=='Alive')/nrow(aa)*100, digits = 2)`%.
...
```{r include=FALSE}
set_flextable_defaults(fonts_ignore = NULL)
tbl_regression(logistic,exponentiate = TRUE) %>% as_flex_table()
ft<-head(tbl_regression(logistic,exponentiate = TRUE)) %>% as_flex_table()
```

```{r echo=FALSE}
ft <- theme_vanilla(ft)
ft <- add_footer_lines(ft, "Adjusted variables with perspective odds ratio, confidence interval and p-value")
ft <- color(ft, part = "footer", color = "#555555")
ft <- set_caption(ft, caption = "Results for adjusted variables")
ft
```
"fill"
```{r}
...
bahb<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=res_survival)) + 
    geom_bar(position = "fill", stat="identity")
oppo_surv<-bahb + rremove("legend.title")
opportunity_survival <- oppo_surv + labs(y = "Percent (%)", x = "Survival (30 days)")
plot(opportunity_survival)


baha<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=intub)) + 
    geom_bar(position = "fill", stat="identity")+
  rremove("ylab")
oppo_intub<-baha + theme(legend.position="none")
opportunity_intub <- baha + labs(y = "Percent (%)", x = "Intubation")
pewp2 <- opportunity_intub + guides(fill=guide_legend(title="OFI"))



bah<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=Gender)) + 
    geom_bar(position = "fill", stat="identity")
oppo_sex<-bah + theme(legend.position="none") 
opportunity_sex <- oppo_sex + labs(y = "Percent (%)", x = "Gender")
pewp1 <- opportunity_sex + guides(fill=guide_legend(title="OFI"))



backa<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=host_care_level)) + 
    geom_bar(position = "fill", stat="identity")+
  rremove("ylab")
oppo_care_level<-backa + theme(legend.position="none")
opportunity_care_level<- oppo_care_level + labs(y = "Percent (%)", x = "Highest hospital care level")
pewp3 <- opportunity_care_level + guides(fill=guide_legend(title="OFI"))



backas<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=rr_rts)) + 
    geom_bar(position = "fill", stat="identity")
oppo_rr<-backas + theme(legend.position="none")
opportunity_rr <- oppo_rr + labs(y = "Percent (%)", x = "Respiratory rate")
pewp4 <- opportunity_rr + guides(fill=guide_legend(title="OFI"))



bahbah<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=sbp_rts)) + 
    geom_bar(position = "fill", stat="identity")+
  rremove("ylab")
oppo_sbp<-bahbah + theme(legend.position="none")
opportunity_sbp <- oppo_sbp + labs(y = "Percent (%)", x = "Systolic blood pressure")
pewp5 <- opportunity_sbp + guides(fill=guide_legend(title="OFI"))



babah<-ggplot(calculation, aes(fill=probYN, y=Percentage, x=gcs_sum)) + 
    geom_bar(position = "fill", stat="identity")+
  rremove("ylab")
oppo_gcs<-babah + theme(legend.position="none")
opportunity_gcs <- oppo_gcs + labs(y = "Percent (%)", x = "Glasgow coma scale")
pewp6 <- opportunity_gcs + guides(fill=guide_legend(title="OFI"))



pp<- ggarrange(pewp1, pewp2, pewp3, pewp4, pewp5, pewp6,
